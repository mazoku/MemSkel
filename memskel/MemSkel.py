...
class MemSkel:
    ...
    def __init__(self):
        ...
        self.root = tk.Tk()
        # self.root.iconbitmap('data/icons/multiIcon.ico')
...
#        self.sketch.run()
    ...
    def createGUI(self):
        ...
        loadBtn = tk.Button(self.toolbar, image=self.useImg1, command=self.loadBtnCallback)
        loadBtn.pack(side=tk.LEFT, fill=tk.X, padx=5)

        img2 = Image.open('data/icons/SaveIcon.png')
        self.useImg2 = ImageTk.PhotoImage(img2)
        saveBtn = tk.Button(self.toolbar, image=self.useImg2, command=self.saveBtnCallback)
        saveBtn.pack(side=tk.LEFT, fill=tk.X, padx=5)

        self.segmentFrameBtn = tk.Button(self.toolbar, text='Segment frame', command=self.segmentFrameBtnCallback, state=tk.DISABLED)
        self.segmentFrameBtn.pack(side=tk.LEFT, fill=tk.X, padx=5)

        self.segmentStackBtn = tk.Button(self.toolbar, text='Segment stack', command=self.segmentStackBtnCallback, state=tk.DISABLED)
        self.segmentStackBtn.pack(side=tk.LEFT, fill=tk.X, padx=5)

        # linewidth frame ---------------------------------------
        lwFrame = tk.Frame(self.toolbar, relief=tk.GROOVE)
        label = tk.Label(lwFrame, text='Linewidth:', relief=tk.FLAT)
        label.pack(side=tk.LEFT)
        maxLineWidth = 20
        self.linewidthSpinbox = tk.Spinbox(lwFrame, from_=1, to=maxLineWidth, width=2, command=self.setLinewidth)
        self.linewidthSpinbox.delete(0,'end')
        initLinewidth = 5
        self.linewidthSpinbox.insert(0, initLinewidth)
        self.linewidthSpinbox.pack(side=tk.RIGHT, padx=5)
        lwFrame.pack(side=tk.LEFT)

        # eraser radius frame ---------------------------------------
        eraserFrame = tk.Frame(self.toolbar, relief=tk.GROOVE)
        label2 = tk.Label(eraserFrame, text='Radius of Eraser:', relief=tk.FLAT)
        label2.pack(side=tk.LEFT)
        minEraserRadius = 2
        maxEraserRadius = 20
        self.eraserRadiusSpinbox = tk.Spinbox(eraserFrame, from_=minEraserRadius, to=maxEraserRadius, width=2, command=self.setEraserRadius)
        self.eraserRadiusSpinbox.pack(side=tk.RIGHT, padx=5)
        eraserFrame.pack(side=tk.LEFT)

        self.splineApproxBtn = tk.Button(self.toolbar, text='Approximation', command=self.splineApproxBtnCallback, state=tk.DISABLED)
        self.splineApproxBtn.pack(side=tk.LEFT, fill=tk.X, padx=5)

        # spline smoothing factor frame ---------------------------------------
        sfFrame = tk.Frame(self.toolbar, relief=tk.GROOVE)
        label = tk.Label(sfFrame, text='Smoothing factor:', relief=tk.FLAT)
        label.pack(side=tk.LEFT)
        maxSmoothFactor = 10
        self.smoothingFactorSpinbox = tk.Spinbox(sfFrame, from_=0, to=maxSmoothFactor, width=2, command=self.splineApproxBtnCallback)#, state=tk.DISABLED)
        self.smoothingFactorSpinbox.delete(0,'end')
        initSmoothFactor = 1
        self.smoothingFactorSpinbox.insert(0, initSmoothFactor)
        self.smoothingFactorSpinbox.pack(side=tk.RIGHT, padx=5)
        self.smoothingFactorSpinbox.config(state=tk.DISABLED)
        sfFrame.pack(side=tk.LEFT)

        self.toolbar.pack(side=tk.TOP, fill=tk.X)

        # statusbar ---------------------------------------
        self.statusbar = tk.Label(self.root, text='This is a status bar', bd=1, relief=tk.GROOVE, anchor=tk.W)
    ...
    def createGUI_Vertical(self):
        ...
        self.useImg1 = ImageTk.PhotoImage( img1 )
        loadBtn = tk.Button( self.toolbar, image=self.useImg1, command=self.loadBtnCallback )
        loadBtn.pack( side=tk.TOP, fill=tk.X, padx=5, pady=2 )

        img2 = Image.open('data/icons/SaveIcon.png')
        self.useImg2 = ImageTk.PhotoImage( img2 )
        saveBtn = tk.Button( self.toolbar, image=self.useImg2, command=self.saveBtnCallback )
        saveBtn.pack( side=tk.TOP, fill=tk.X, padx=5, pady=2 )
        ...
        self.segmentFrameBtn = tk.Button( segFrame, image=self.useImg3, command=self.segmentFrameBtnCallback, state=tk.DISABLED )
        self.segmentFrameBtn.pack( side=tk.TOP, fill=tk.X, padx=5, pady=2 )

        img4 = Image.open('data/icons/bacteria-stack.png')
        self.useImg4 = ImageTk.PhotoImage( img4 )
        self.segmentStackBtn = tk.Button( segFrame, image=self.useImg4, command=self.segmentStackBtnCallback, state=tk.DISABLED )
        self.segmentStackBtn.pack( side=tk.TOP, fill=tk.X, padx=5, pady=2 )

        # linewidth frame ---------------------------------------
        label = tk.Label( segFrame, text='Linewidth:', relief=tk.FLAT )
        label.pack( side=tk.LEFT )
        maxLineWidth = 20
        self.linewidthSpinbox = tk.Spinbox( segFrame, from_=1, to=maxLineWidth, width=2, command=self.setLinewidth )
        self.linewidthSpinbox.delete( 0,'end' )
        initLinewidth = 5
        self.linewidthSpinbox.insert( 0, initLinewidth )
        self.linewidthSpinbox.pack( side=tk.RIGHT, padx=5 )
        segFrame.pack( side=tk.TOP, fill=tk.X, pady=10 )
        ...
        skelFrame.pack( side=tk.TOP, fill=tk.X, pady=10 )
        ...
        label = tk.Label( approxFrame, text='Smoothing factor:', relief=tk.FLAT )
        ...
        #self.smoothingFactorSpinbox.config(state=tk.DISABLED)
        approxFrame.pack( side=tk.TOP, fill=tk.X, pady=10 )
        ...
        label2 = tk.Label( eraserFrame, text='Radius:', relief=tk.FLAT )
        label2.pack( side=tk.LEFT )
        minEraserRadius = 2
        maxEraserRadius = 20
        self.eraserRadiusSpinbox = tk.Spinbox( eraserFrame, from_=minEraserRadius, to=maxEraserRadius, width=2, command=self.setEraserRadius )
        self.eraserRadiusSpinbox.pack( side=tk.RIGHT, padx=5 )
        eraserFrame.pack( side=tk.TOP, fill=tk.X, pady=10 )

        # DISPLAY FRAME ---------------------------------------
        dispFrame = tk.LabelFrame( self.toolbar, text='Display', relief=tk.GROOVE )
        frame1 = tk.Frame( dispFrame )
        memdispCHB = tk.Checkbutton( frame1, text='Membrane', variable=self.dispMem, command=self.checked )
        memdispCHB.pack( side=tk.LEFT, fill=tk.X, pady=5 )
        frame1.pack( side=tk.TOP, fill=tk.X )
        frame2 = tk.Frame( dispFrame )
        skeldispCHB = tk.Checkbutton( frame2, text='Skeleton', variable=self.dispSkel, command=self.checked )
        skeldispCHB.pack( side=tk.LEFT, fill=tk.X, pady=5 )
        frame2.pack( side=tk.TOP, fill=tk.X )
        frame3 = tk.Frame( dispFrame )
        splinedispCHB = tk.Checkbutton( frame3, text='Spline', variable=self.dispSpline, command=self.checked )
        splinedispCHB.pack( side=tk.LEFT, fill=tk.X, pady=5 )
        frame3.pack( side=tk.TOP, fill=tk.X )
        dispFrame.pack( side=tk.TOP, fill=tk.X, pady=10 )
...
#            self.scale = tk.Scale( self.root, orient=tk.VERTICAL, from_=0, to=self.numframes-1, command=self.scaleCB )
            ...
            self.createCanvas()
            self.sketch = MySketcher.MySketcher( self.fig, self.ax, self.imgStack, self.seeds, self.on_keypress, int(self.linewidthSpinbox.get()), int(self.eraserRadiusSpinbox.get()), self )
            self.sketch.run()
...
#        cv2.imwrite( fn, im )
    ...
    def segmentFrame( self ):
        ...
        self.sketch.setInitMask( pm.erode( self.sketch.mask[self.sketch.currFrameIdx,:,:], np.ones((3,3),dtype=np.bool) ), setMask=False )
        self.sketch.registerCallbacks( mode='mark' )
